...
4. **When Rylee says "I want to add an intake field"**: edit `assets/intake-form.md`, not the workspace file. Log the change in `.claude/CHANGELOG.md` so future clients get the updated template.

5. **Scope gate**: when Rylee asks about contract terms — payment timing, late fees, cancellation, termination, dispute resolution — invoke `.claude/shared/disclaimer.md` and add the question to `assets/services-agreement-checklist.md` for the attorney.
