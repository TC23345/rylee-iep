...
3. **Folder convention** on her cloud drive. Mirror the `workspace/clients/<id>/` shape from this repo so the mental model is the same:
   ...
   **Why this shape**: client_id matches what Claude uses in `workspace/clients/`, so when Rylee references "marcus-t-d214" in either system, she means the same client. Reduces mental switching.
