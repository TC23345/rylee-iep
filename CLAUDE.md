...
- **Case types.** Eleven types in `lib/entry-schema.ts` (case work vs other time). A row counts as a case only when it has a case number and is not lunch; `lib/entries.ts` mirrors that rule in its aggregates. Notes are free text; there are no per-type note templates.
