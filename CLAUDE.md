...
The root-level Next.js app (deployed to https://acentra-case-tracker.up.railway.app) is **no longer the course dashboard**. It is a daily case log for Rylee's day job, modelled on the Excel workbook she kept by hand: a "Daily Case Counts" sheet plus one sheet per month with rows of Date · Case # · Case Type · Start · End · Duration · Notes. `README.md` describes every feature and has the file map.
...
- **Case types.** Per log and editable from the pencil on the day table's Type header. Eleven built-ins in `lib/case-types.ts`; overrides and added types in Mongo `case_types` (removal is a soft flag). Rows store the type's stable `key`, never its label. Each type has a category: `case` (needs a case #), `other`, `break` (never counts). A row counts as a case when it has a case number and its type is not `break`; `lib/entries.ts` mirrors that rule in its aggregates via `getBreakKeys`. Client components read types with `useCaseTypes()`; never hard-code a type key or "lunch" check. Notes are free text; there are no per-type note templates.
- **Importing spreadsheets.** `docs/import-format.md` is the contract; `docs/import-samples/` are the reference files. When converting one of Rylee's unstructured sheets, produce a workbook matching them exactly, then use Import Excel on a month page (Case Tracker menu). The importer dedupes on day + case number + start time.
- **UI conventions.** Rows edit inline and save on blur. Delete is hold-to-delete with an Undo toast, no confirm dialog. The Next button and submit button in the add dialog must stay separately keyed (React reuses the node otherwise and a real click submits). The type legend and the day log belong on the month pages; `/` redirects to today's log (`/month/<current month>`); the "Calendar" tab (`/calendar`) is tiles and the month card, and the daily sheet is its own "Case Counts" tab (`/counts`). Colours and fonts follow `docs/acentra-design.md`; no hex literals in components.
- **Time and the running clock.** Time follows the user's machine; nothing is pinned to a timezone. The browser uses its own clock (`nowTime`, `localTodayIso`), and server code gets "today" from `userToday()` in `lib/timezone.ts`, which reads the `tz` cookie set by `TimeZoneCookie`. Never call `todayIso()` bare in a page. A row dated today with a Start and no End is a running case: the day log shows a live clock in its End cell, and clicking it stamps End. New rows for today start at "now" with End blank. Running cases never auto-stop.
- **Case numbers.** Nine digits (`CASE_NUMBER_LENGTH` in `lib/entry-schema.ts`). The rule applies when a row is created or its number changes. It does not apply to Undo (`createEntry(values, { restore: true })`), to edits that leave the number alone, or to the spreadsheet importer, so older and backfilled rows keep their numbers.
